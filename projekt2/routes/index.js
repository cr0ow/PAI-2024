...
indexRouter.get('/', async (req, res) => {
  let items = await models.Thing.findAll({ order: ['name'] })
  res.render('index', { title: 'Rezerwacje', items: items })
})

indexRouter.get('/bookings/:id/', async (req, res) => {
  let rows = await models.Booking.findAll({ where: { thingId: req.params.id } })
  ...
  let toUpdate = []
  rows.forEach((row_) => {
    let startDate = new Date(row_.bookingStart);
    let finishDate = new Date(row_.bookingFinish);
    let dates = []
  ...
  let thing = await models.Thing.findByPk(req.params.id)
  res.render('bookings', { thing: thing, data: data})
})

indexRouter.post('/booking-confirmation', (req, res) => {
  let body = req.body
  ...
  toInsert.forEach(it => {
    models.Booking.create({bookingStart: it.start, bookingFinish: it.finish, whoBooked: it.whoBooked, ThingId: it.idThing})
